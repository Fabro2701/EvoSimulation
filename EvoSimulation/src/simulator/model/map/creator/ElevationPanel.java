...
/*
 * public class ElevationPanel extends javax.swing.JPanel { private Controller
 * ctrl;
 * 
 * 
 * public ElevationPanel() { initComponents(); ctrl = new Controller();
 * modificationPanel.setViewportView(ctrl.img);
 * viewPanel.setViewportView(ctrl.viewObserver); }
 * 
 * private class Controller implements MouseListener, MouseMotionListener {
 * private Graphics2D bufferGraphics; private BufferedImage bufferImage; private
 * int strokeSize, intensity; private int width, height; private
 * ModificationImage img; private Point current; private ViewImage viewObserver;
 * 
 * private float xSkew = 1.0f; private float ySkew = 1.0f; int originX = 0; int
 * originY = 0; int xs = 10;
 * 
 * public Controller() { width = 800; height = 800;
 * 
 * strokeSize = 50; intensity = 50;
 * 
 * bufferImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
 * bufferGraphics = bufferImage.createGraphics(); bufferGraphics.setColor(new
 * Color(255, 255 / 2, 0, 255)); bufferGraphics.fillRect(0, 0, width, height);
 * 
 * img = new ModificationImage(new ImageIcon(bufferImage), this); current = new
 * Point();
 * 
 * viewObserver = new ViewImage(); viewObserver.update(bufferImage); }
 * 
 * @Override public void mousePressed(MouseEvent e) {
 * current.setLocation(e.getPoint()); }
 * 
 * @Override public void mouseDragged(MouseEvent e) { // TODO Auto-generated
 * method stub if (SwingUtilities.isLeftMouseButton(e)) {
 * bufferGraphics.setColor(new Color(255, 255, 0, intensity)); } else {
 * bufferGraphics.setColor(new Color(255, 0, 0, intensity)); }
 * 
 * bufferGraphics.fillOval(e.getX() - strokeSize / 2, e.getY() - strokeSize / 2,
 * strokeSize, strokeSize); current.setLocation(e.getPoint()); img.setIcon(new
 * ImageIcon(bufferImage)); viewObserver.update(bufferImage);
 * 
 * }
 * 
 * private class ViewImage extends JLabel implements Scrollable { int
 * reducedWidth, reducedHeight; BufferedImage buffer;
 * 
 * public ViewImage() { reducedWidth = 100; reducedHeight = 100; KeyListener
 * keyListener = new KeyListener() {
 * 
 * @Override public void keyPressed(KeyEvent ke) { int c = ke.getKeyCode();
 * switch (c) { case KeyEvent.VK_W: originY -= 10; break; case KeyEvent.VK_S:
 * originY += 10; break; case KeyEvent.VK_A: originX -= 10; break; case
 * KeyEvent.VK_D: originX += 10; break; default: } update(buffer);
 * 
 * }
 * 
 ...
 * 
 * }; this.addKeyListener(keyListener); this.setFocusable(true);
 * this.requestFocusInWindow(); MouseAdapter mouse = new MouseAdapter() {
 * boolean pressed = false; Point current = null;
 * 
 * @Override public void mouseClicked(MouseEvent e) { focus(); }
 * 
 * @Override public void mouseWheelMoved(MouseWheelEvent e) { if
 * (e.getWheelRotation() != 0) { xs += e.getWheelRotation(); update(buffer); } }
 * 
 * @Override public void mousePressed(MouseEvent e) { if (!pressed) { pressed =
 * true; current = e.getPoint(); } // }
 * 
 * @Override public void mouseReleased(MouseEvent e) { pressed = false;
 * 
 * }
 * 
 * @Override public void mouseDragged(MouseEvent e) { if (pressed) { Point dir =
 * e.getPoint(); if (dir.equals(current)) return; int dx = dir.x - current.x;
 * int dy = dir.y - current.y;
 * 
 * float decreaseFactor = 1000.0f; xSkew += (float) dx / decreaseFactor; ySkew
 * += (float) dy / decreaseFactor; // System.out.println(xSkew+" "+ySkew);
 * update(buffer); } } }; this.addMouseListener(mouse);
 * this.addMouseMotionListener(mouse); this.addMouseWheelListener(mouse); }
 * 
 * public void focus() { this.requestFocusInWindow(); }
 * 
 * private BufferedImage reduceImage(BufferedImage img, int w, int h) { Image
 * imgS = img.getScaledInstance(w, h, Image.SCALE_SMOOTH); BufferedImage image2
 * = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB); Graphics2D g2d =
 * (Graphics2D) image2.createGraphics(); g2d.drawImage(imgS, 0, 0, w, h, null);
 * return image2; }
 * 
 * public void update(BufferedImage image) { buffer = image; BufferedImage
 * transformedImage = new BufferedImage(width, height,
 * BufferedImage.TYPE_INT_ARGB); BufferedImage reducedImage = reduceImage(image,
 * reducedWidth, reducedHeight); Graphics2D g2d =
 * transformedImage.createGraphics(); g2d.setColor(Color.black); g2d.fillRect(0,
 * 0, width, height); g2d.setColor(Color.green);
 * 
 * Point2D point1; Point2D point2;
 * 
 * float inskew = 2.0f;
 * 
 * for (int i = 0; i < reducedImage.getHeight(); i++) { for (int j = 0; j <
 * reducedImage.getWidth(); j++) { point1 = transform3D(new Point3D(j * xs,
 * (int) (new Color(reducedImage.getRGB(j, i), true).getGreen() * inskew), i *
 * xs)); if (j < reducedImage.getWidth() - 1) {
 * 
 * point2 = transform3D(new Point3D((j + 1) * xs, (int) (new
 * Color(reducedImage.getRGB(j + 1, i), true).getGreen() * inskew), i * xs)); //
 * g2d.fillOval(point1.x, point1.y, 20, 20);
 * 
 * // System.out.println(point1.x + originX+" "+point1.y + originY+" "+point2.x
 * + // originX+" "+point2.y + originY); // try { g2d.drawLine(point1.x +
 * originX, point1.y + originY, point2.x + originX, point2.y + originY);
 * 
 * } if (i < reducedImage.getHeight() - 1) {
 * 
 * point2 = transform3D(new Point3D(j * xs, (int) (new
 * Color(reducedImage.getRGB(j, i + 1), true).getGreen() * inskew), (i + 1) *
 * xs));
 * 
 * g2d.drawLine(point1.x + originX, point1.y + originY, point2.x + originX,
 * point2.y + originY); } } }
 ...
 * this.setIcon(new ImageIcon(transformedImage));// transform }
 * 
 * public Point2D transform3D(Point3D point3D) { return new Point2D((int)
 * ((point3D.x + point3D.z) * xSkew), (int) (((-point3D.y) + point3D.z -
 * point3D.x) * ySkew)); // return new Point2D((int)((point3D.x +
 * point3D.z)*xSkew), (int) (((-point3D.y) // + point3D.z) * ySkew)); }
 * 
 * public class Point2D { public Point2D(int x, int y) { this.x = x; this.y = y;
 * }
 * 
 * int x = 0; int y = 0; }
 * 
 * public class Point3D { public Point3D(int x, int y, int z) { this.x = x;
 * this.y = y; this.z = z; }
 * 
 ...
 * }
 * 
 * private class ModificationImage extends JLabel implements Scrollable { public
 * ModificationImage(ImageIcon icon, Controller ctrl) { super(icon);
 ...
 * }
 * 
 * public void setIntensity(int value) { intensity = value;
 * 
 * }
 * 
 * public void setStrokeSize(int value) { strokeSize = value; } }
 * 
 * 
 * 
 ...
 * javax.swing.JPanel(); jLabel2 = new javax.swing.JLabel(); jsStrokeSize = new
 * javax.swing.JSlider(); jLabel3 = new javax.swing.JLabel(); jsIntensisty = new
 * javax.swing.JSlider(); viewPanel = new javax.swing.JScrollPane(); jPanel2 =
 * new javax.swing.JPanel(); jbZoomIn = new javax.swing.JButton(); jbZoomOut =
 ...
 * jLabel3.setText("Intensity");
 * 
 * jsIntensisty.setMinimum(1); jsIntensisty.addChangeListener(new
 * javax.swing.event.ChangeListener() { public void
 * stateChanged(javax.swing.event.ChangeEvent evt) {
 * jsIntensistyStateChanged(evt); } });
 * 
 ...
 * javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
 * .addComponent(jsIntensisty, javax.swing.GroupLayout.DEFAULT_SIZE,
 * javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)) .addGap(15, 15,
 * 15)));
 * 
 * viewPanel.setBorder(new javax.swing.border.MatteBorder(null));
 * 
 * jPanel2.setBorder(new javax.swing.border.MatteBorder(null));
 * 
 * jbZoomIn.setText("Zoom in"); jbZoomIn.setToolTipText("");
 * jbZoomIn.addActionListener(new java.awt.event.ActionListener() { public void
 * actionPerformed(java.awt.event.ActionEvent evt) {
 * jbZoomInActionPerformed(evt); } });
 * 
 * jbZoomOut.setText("Zoom out"); jbZoomOut.addActionListener(new
 ...
 * addComponent(jbZoomIn) .addGap(39, 39,
 * 39).addComponent(jbZoomOut).addContainerGap(518, Short.MAX_VALUE)));
 ...
 * .LEADING) .addComponent(jbZoomOut, javax.swing.GroupLayout.DEFAULT_SIZE,
 * javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
 * .addComponent(jbZoomIn, javax.swing.GroupLayout.DEFAULT_SIZE,
 ...
 * .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
 * .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.
 * LEADING, false) .addComponent(viewPanel).addComponent(jPanel2,
 * javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
 * Short.MAX_VALUE)) .addGap(0, 29, Short.MAX_VALUE)));
 ...
 * .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.
 * LEADING, false) .addComponent(viewPanel).addComponent(modificationPanel,
 * javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE,
 * Short.MAX_VALUE))
 * .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
 * .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.
 ...
 * 
 * private void jsStrokeSizeStateChanged(javax.swing.event.ChangeEvent evt) {
 * ctrl.setStrokeSize(jsIntensisty.getValue()); // TODO add your handling code
 * here: }
 * 
 * private void jsIntensistyStateChanged(javax.swing.event.ChangeEvent evt) {
 * ctrl.setIntensity(jsIntensisty.getValue()); }
 * 
 * private void jbZoomOutActionPerformed(java.awt.event.ActionEvent evt) { //
 * TODO add your handling code here: }
 * 
 * // Variables declaration - do not modify private javax.swing.JLabel jLabel2;
 * private javax.swing.JLabel jLabel3; private javax.swing.JPanel jPanel1;
 * private javax.swing.JPanel jPanel2; private javax.swing.JButton jbZoomIn;
 * private javax.swing.JButton jbZoomOut; private javax.swing.JSlider
 * jsIntensisty; private javax.swing.JSlider jsStrokeSize; private
 * javax.swing.JScrollPane modificationPanel; private javax.swing.JScrollPane
 * viewPanel; // End of variables declaration }
 */
